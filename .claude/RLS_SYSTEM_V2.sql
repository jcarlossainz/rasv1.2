...
-- Eliminar políticas existentes de servicios_inmueble
DROP POLICY IF EXISTS "servicios_inmueble_select" ON servicios_inmueble;
DROP POLICY IF EXISTS "servicios_inmueble_insert" ON servicios_inmueble;
DROP POLICY IF EXISTS "servicios_inmueble_update" ON servicios_inmueble;
DROP POLICY IF EXISTS "servicios_inmueble_delete" ON servicios_inmueble;

-- Eliminar políticas existentes de documentos
...
DROP POLICY IF EXISTS "contactos_select" ON contactos;
DROP POLICY IF EXISTS "contactos_insert" ON contactos;
DROP POLICY IF EXISTS "contactos_update" ON contactos;
DROP POLICY IF EXISTS "contactos_delete" ON contactos;
...
-- NOTA: tickets usa "propiedad_id" (no property_id)
...
-- ============================================================================
-- PASO 7: POLÍTICAS PARA CALENDAR_EVENTS
-- ============================================================================
-- Acceso: administrador, propietario, supervisor (NO promotor)
-- NOTA: calendar_events usa "propiedad_id" (no property_id)
...
-- ============================================================================
-- PASO 8: POLÍTICAS PARA PROPERTY_IMAGES
-- ============================================================================
...
-- ============================================================================
-- PASO 9: POLÍTICAS PARA PROPERTY_ARCHIVOS (Archivero)
-- ============================================================================
...
-- ============================================================================
-- PASO 10: POLÍTICAS PARA PROPERTY_INVENTORY
-- ============================================================================
-- Acceso: administrador, propietario, supervisor (NO promotor)
...
-- ============================================================================
-- PASO 11: POLÍTICAS PARA CUENTAS (Balance)
-- ============================================================================
-- Acceso: SOLO el usuario dueño de la cuenta
-- NOTA: cuentas usa "user_id" y "propiedades_ids" (array), no property_id simple

-- SELECT: Solo el dueño de la cuenta
CREATE POLICY "cuentas_select" ON cuentas
    FOR SELECT USING (user_id = auth.uid());

-- INSERT: Solo puede crear cuentas para sí mismo
...
-- ============================================================================
-- PASO 12: POLÍTICAS PARA INGRESOS (Balance)
-- ============================================================================
-- Acceso: SOLO administrador y propietario (NO supervisor, NO promotor)
...
-- ============================================================================
-- PASO 13: POLÍTICAS PARA USER_DASHBOARD_CONFIG (Config)
-- ============================================================================
...
-- ============================================================================
-- PASO 14: POLÍTICAS PARA SERVICIOS_INMUEBLE
-- ============================================================================
-- Acceso: administrador, propietario, supervisor (NO promotor)

-- SELECT
CREATE POLICY "servicios_inmueble_select" ON servicios_inmueble
    FOR SELECT USING (
        user_has_role(propiedad_id, ARRAY['administrador', 'propietario', 'supervisor'])
    );

-- INSERT
CREATE POLICY "servicios_inmueble_insert" ON servicios_inmueble
    FOR INSERT WITH CHECK (
        user_has_role(propiedad_id, ARRAY['administrador', 'propietario'])
    );

-- UPDATE
CREATE POLICY "servicios_inmueble_update" ON servicios_inmueble
    FOR UPDATE USING (
        user_has_role(propiedad_id, ARRAY['administrador', 'propietario'])
    );

-- DELETE
CREATE POLICY "servicios_inmueble_delete" ON servicios_inmueble
    FOR DELETE USING (
        user_has_role(propiedad_id, ARRAY['administrador', 'propietario'])
    );

-- ============================================================================
-- PASO 15: POLÍTICAS PARA DOCUMENTOS
-- ============================================================================
-- Acceso: administrador, propietario (documentos pueden estar asociados a propiedad o ticket)

-- SELECT
CREATE POLICY "documentos_select" ON documentos
    FOR SELECT USING (
        -- Documentos de propiedades donde tengo acceso
        (propiedad_id IS NOT NULL AND user_has_role(propiedad_id, ARRAY['administrador', 'propietario']))
        OR
        -- Documentos de tickets donde tengo acceso
        (ticket_id IS NOT NULL AND EXISTS (
            SELECT 1 FROM tickets t
            WHERE t.id = ticket_id
            AND user_has_role(t.propiedad_id, ARRAY['administrador', 'propietario', 'supervisor'])
        ))
    );
...
CREATE POLICY "documentos_update" ON documentos
    FOR UPDATE USING (
        ...
        (ticket_id IS NOT NULL AND EXISTS (
            SELECT 1 FROM tickets t
            WHERE t.id = ticket_id
            AND user_has_role(t.propiedad_id, ARRAY['administrador', 'propietario'])
        ))
    );
...
-- ============================================================================
-- PASO 16: POLÍTICAS PARA CONTACTOS
-- ============================================================================
-- Acceso: Solo el usuario dueño de los contactos
-- NOTA: contactos está asociado a user_id, no a propiedad

-- SELECT: Solo el dueño
CREATE POLICY "contactos_select" ON contactos
    FOR SELECT USING (user_id = auth.uid());

-- INSERT: Solo puede crear contactos para sí mismo
