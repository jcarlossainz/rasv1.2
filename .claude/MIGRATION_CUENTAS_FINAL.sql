-- ============================================================================
-- 🏢 RAS - MIGRACIÓN: Sistema de Cuentas Bancarias e Ingresos (VERSIÓN FINAL)
-- ============================================================================
-- Fecha: 21 de Noviembre 2025
-- Descripción: Agrega tablas para cuentas bancarias, ingresos y relaciones
-- VERSIÓN FINAL: Elimina triggers primero, luego funciones
-- ============================================================================

-- ============================================================================
-- PASO 0: LIMPIAR TODO LO EXISTENTE (ORDEN CORRECTO)
-- ============================================================================

-- 1. Eliminar TRIGGERS primero (dependen de funciones)
...
-- 2. Eliminar FUNCIONES (ahora ya no tienen dependencias)
...
-- 3. Eliminar VISTA (si existe)
DROP VIEW IF EXISTS v_movimientos_cuenta;
...
BEGIN
  ...
  IF OLD.pagado = TRUE AND NEW.pagado = FALSE AND OLD.cuenta_id IS NOT NULL AND OLD.monto_real IS NOT NULL THEN
    UPDATE cuentas_bancarias
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    -- Revertir el ingreso anterior si había cuenta
    ...
    -- Aplicar el nuevo ingreso si hay cuenta
...
-- Función para generar fechas de pago automáticas
...
BEGIN
  ...
  FOR i IN 1..p_cantidad_meses LOOP
    ...
    SELECT EXISTS (
      ...
      WHERE servicio_id = p_servicio_id AND fecha_pago = v_fecha_pago
    ) INTO v_existe;

    -- Insertar solo si no existe
    IF NOT v_existe THEN
      INSERT INTO fechas_pago_servicios (
        servicio_id, propiedad_id, fecha_pago, monto_estimado, pagado
      ) VALUES (
        p_servicio_id, v_servicio.propiedad_id, v_fecha_pago, v_servicio.monto, FALSE
      );
...
-- ============================================================================
-- PASO 6: CREAR TRIGGERS (DESPUÉS de las funciones)
-- ============================================================================

-- Trigger para updated_at en fechas_pago_servicios
CREATE TRIGGER update_fechas_pago_updated_at
  BEFORE UPDATE ON fechas_pago_servicios
  FOR EACH ROW
...
-- Trigger para updated_at en ingresos
CREATE TRIGGER update_ingresos_updated_at
  BEFORE UPDATE ON ingresos
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();

-- Trigger para actualizar balance con ingresos
...
-- ============================================================================
-- PASO 7: CREAR VISTA CONSOLIDADA
-- ============================================================================
...
COMMENT ON TABLE cuentas_bancarias IS 'Cuentas bancarias, tarjetas y efectivo';
COMMENT ON TABLE ingresos IS 'Registro de ingresos (rentas, depósitos, ventas)';
COMMENT ON COLUMN cuentas_bancarias.balance_actual IS 'Balance calculado por triggers';
COMMENT ON VIEW v_movimientos_cuenta IS 'Vista consolidada de movimientos';
