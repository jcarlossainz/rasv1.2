...
export function AssistantChat({ mode = 'floating', onClose, avatarSrc, avatarLabel, userId }: AssistantChatProps) {
  ...
  // Contenedor de límites para el drag (pantalla completa)
  if (mode === 'floating') {
    return (
      <>
        {/* Contenedor de límites - cubre toda la pantalla */}
        <div
          ref={constraintsRef}
        ...
        {isMinimized ? (
          ...
          >
            <button
              ...
              className="w-14 h-14 bg-gradient-to-br from-slate-700 to-slate-900 rounded-full shadow-lg flex items-center justify-center text-white hover:shadow-xl transition-shadow overflow-hidden"
            >
              {avatarSrc ? (
          ...
          <motion.div
            ...
            className="fixed top-20 right-4 w-96 max-h-[500px] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden z-50"
          >
            {/* Header - Área de arrastre */}
            <div
              className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-slate-700 to-slate-800 text-white cursor-grab active:cursor-grabbing"
  ...
  // Modo embedded (sin drag)
  return (
    <div className="w-full h-full bg-white rounded-xl border border-gray-200 flex flex-col overflow-hidden">
      {/* Header */}
